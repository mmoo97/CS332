...
int main(int argc, char **argv) {
    ...
    while (fgets(line, BUFSIZ, fp1) != NULL) {
      ...
      if (pid == 0) { /* this is child process */
        ...
        if ((fdout = open(snum, O_CREAT | O_APPEND | O_WRONLY, 0755)) == -1) {
           printf("Error opening file stdout.txt for output\n");
           exit(-1);
        }

        if ((fderr = open(snum2, O_CREAT | O_APPEND | O_WRONLY, 0755)) == -1) {
           printf("Error opening file stderr.txt for output\n");
           exit(-1);
        }
